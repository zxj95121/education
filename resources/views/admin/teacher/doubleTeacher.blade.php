                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        @if($value->status!=1)
                                                        	...
                                                        	<td class="caozuo">
                                                        		<a href="/admin/teacheroneHide?id={{$value->id}}"><span class="label label-success" tid="{{$value->id}}" style="margin-right: 4px;">显示</span></a>
                                                        @else
                                                        	<td><font><font class=""><span class="label label-success">显示</span></font></font></td>
                                                        	<td class="caozuo">
                                                        		<a href="/admin/teacheroneHide?id={{$value->id}}"><span class="label label-danger" tid="{{$value->id}}" style="margin-right: 4px;">隐藏</span></a>
                                                        @endif
                                                        	<a href="/admin/teacherTwo?id={{$value->id}}"><span class="label label-info" tid="{{$value->id}}" style="margin-right: 4px;">设置</span></a>
                                                        	<a href="/admin/teacheroneEdit?id={{$value->id}}"><span class="label label-primary" tid="{{$value->id}}" style="margin-right: 4px;">修改</span></a>
...
<script>
	$(function(){
        ...
        $('#add').click(function(){
            window.location.href="{{URL('admin/teacheroneAdd')}}";
        })
        $('.delete').click(function(){
			...
			window.layer.confirm('确定要删除该'+otitle+'类吗?',{
				...
				window.location.href="/admin/teacheroneDelete?id="+tid;
			},function(){
			})
			return false;
        })
