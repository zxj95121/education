...
(function(root, factory) {
    ...
    //初始化并创建元素
    jefn.init = function() {
        var _this = this, opts = _this.opts, inelem = _this.elCell;
        var jeuuid = function() {
    ...
    jefn.onBind = function(uuid) {
        ...
        $("#jeselbox" + uuid).on("click", function() {
            var openCell = _this.setCentent(uuid), bthis = $(this);
            //模拟列表点击事件-赋值
            openCell.find("dd").on("click", function() {
                var othis = $(this), index = parseInt(othis.attr("item")), vals = othis.attr("val");
                othis.addClass(opts.currCls).siblings().removeClass(opts.currCls);
                $("#jeselbox" + uuid).text(vals);
                //同时改变select的选中状态
                ...
                if (opts.itemfun != undefined && typeof opts.itemfun === "function") {
                    opts.itemfun(inelem, index, vals);
                }
            });
            //执行方向定位
    ...
    jefn.setCentent = function(uuid) {
        var _this = this, opts = _this.opts, selHtml = "", s = 0, selbox = $("#jeselbox" + uuid);
        //创建并生成下拉列表
        ...
        $("option, optgroup", _this.elCell).each(function() {
            //判断是否为option
            if ($(this).is("option")) {
                var groupCell = $(this).parent().is("optgroup") ? "group" :"", text = $(this).text();
                //判断初始化是否为选中状态
                if ($(this).is(":selected")) {
    ...
    jefn.searchList = function (openCell, uuid) {
        var _this = this, opts = _this.opts, selbox = $("#jeselbox" + uuid);
        if (opts.sosList){
            openCell.find("input").on("input cut focus keydown keyup paste",function () {
                var sothat = $(this), soVal = sothat.val(), opendlCell = openCell.find("dl");
                //判断不为空的条件下
                if (soVal != "") {
